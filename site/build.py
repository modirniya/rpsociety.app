...
:root{
  ...
  --indie:#e0873a; --indie-soft:rgba(224,135,58,.13);
...
@media (prefers-color-scheme:light){
  :root:not([data-theme="dark"]){
    ...
    --indie:#9a4a0c; --indie-soft:rgba(154,74,12,.09);
...
/* ── role generator — theme-aware via the site's own tokens ───────────────────────────── */
...
.rt .rt-side{display:inline-block;font-size:10px;letter-spacing:.1em;text-transform:uppercase;font-weight:700;
  padding:2px 7px;border-radius:5px;margin:3px 0 2px}
.rt .rt-side-town{color:var(--town);background:var(--town-soft)}
.rt .rt-side-mafia{color:var(--mafia);background:var(--mafia-soft)}
.rt .rt-side-independent{color:var(--indie);background:var(--indie-soft)}
.rt .rt-role-c{display:flex;flex-direction:column;align-items:center;gap:2px}
.rt .rt-editlink{font-size:12.5px;padding:2px}
.rt .rt-role-open{display:block;border-color:var(--accent)}
.rt .rt-add{width:100%;border-style:dashed}
.rt .rt-edit{display:grid;gap:12px}
.rt .rt-f{display:grid;gap:5px;min-width:0}
.rt .rt-f>span{font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:var(--ink-faint);font-weight:700}
.rt .rt-f input,.rt .rt-f textarea{width:100%;box-sizing:border-box;padding:9px 11px;border-radius:9px;border:1px solid var(--line);
  background:var(--raised-2);color:var(--ink);font:inherit;font-size:15px}
.rt .rt-f textarea{resize:vertical;min-height:70px}
.rt .rt-f input[aria-invalid="true"]{border-color:var(--mafia)}
.rt .rt-f-row{display:grid;grid-template-columns:84px 1fr;gap:12px;align-items:start}
.rt .rt-f-icon input{text-align:center;font-size:18px}
.rt .rt-dots-pick{display:flex;flex-wrap:wrap;gap:7px}
.rt .rt-dot{width:28px;height:28px;border-radius:50%;border:2px solid transparent;cursor:pointer;padding:0}
.rt .rt-dot.on{border-color:var(--ink);box-shadow:0 0 0 2px var(--raised)}
.rt .rt-segs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}
.rt .rt-seg{padding:9px 6px;border-radius:9px;border:1px solid var(--line);background:var(--raised-2);color:var(--ink-dim);
  font:inherit;font-size:13.5px;font-weight:600;cursor:pointer}
.rt .rt-seg.on{border-color:var(--accent);color:var(--ink);background:var(--accent-soft)}
.rt .rt-check{display:flex;align-items:center;gap:9px;font-size:14.5px}
.rt .rt-check input{width:18px;height:18px}
.rt .rt-danger{color:var(--mafia)}
.rt .rt-card-team.rt-side-town,.rt .rt-card-team.rt-side-mafia,.rt .rt-card-team.rt-side-independent{background:none;padding:0}
...
@media(max-width:480px){.rt .rt-who{font-size:26px}.rt .rt-chip input{width:100%}
...
<section>
  ...
  <ul>
    ...
    <li><strong>Never announce a town power role.</strong> A Doctor who says "I am the Doctor" is
    dead that night, and so is a Detective, a Sniper, or a Die-hard whose armour is spent. You have
    not proved anything to the town — you have told the mafia who to remove first.</li>
    <li><strong>A Detective says the name, never the source.</strong> Even holding a result,
    explaining how you know ends your nights, and tomorrow's inquiry is usually worth more than
    today's accusation.</li>
    <li><strong>"I am town" is free, and means nothing.</strong> Anybody may say it and the mafia
    say it with the same confidence you do. It is the one claim that carries no information at
    all.</li>
    <li><strong>A mafioso can claim a role, and nobody can check.</strong> He can announce he is the
    Detective and deliver a confident result, because no town role can prove anything either. Treat
    every role claim as weak evidence, whoever makes it.</li>
...
def build_tool_assets() -> None:
    ...
    The sources live in site/js/ so they can be tested with node (`node --test site/test_deck.js`);
    ...
    """
    global JS_VERSION
    deck = (ROOT / "site" / "js" / "deck.js").read_text(encoding="utf-8")
    ui = (ROOT / "site" / "js" / "generator.js").read_text(encoding="utf-8")
    JS_VERSION = hashlib.sha1((deck + ui).encode("utf-8")).hexdigest()[:8]
    write("assets/js/deck.js", deck)
    ...
    write("assets/manifest.webmanifest", json.dumps({
        ...
        "description": "Deal Mafia roles in secret from one phone — the game's own or your own.",
    ...
    precache = [
        "/tools/role-generator/",
        "/assets/site.css", "/assets/fonts.css",
        f"/assets/js/deck.js?v={JS_VERSION}", f"/assets/js/generator.js?v={JS_VERSION}",
        "/assets/icon-192.png", "/assets/icon-512.png", "/assets/favicon.png",
        "/assets/favicon-48.png", "/assets/favicon-96.png",
    ] + fonts
...
def tool_head() -> str:
    ...
    return (
        ...
        f'<script src="/assets/js/deck.js?v={v}" defer></script>'
...
def tool_mount() -> str:
    return (
        '<div id="tool" class="rt" aria-live="polite">'
...
APP_FUNNEL = f"""
<section>
  <h2>Or skip the narrator entirely</h2>
  <p>Everyone in one room? This deck is for you. Apart? The game itself deals in secret, runs
...
<section class="hero">
  ...
  <p class="lede">A deck of role cards on one phone. Set the table, then pass the phone: each
  player sees only their own role, and the mafia see their team. Use the game's own roles, rename
  them, or write your own — in any language, on the town, the mafia or an independent side.
  Nothing ever leaves your browser.</p>
</section>
...
<section>
  ...
  <ol class="steps">
    <li><strong>Set up the table.</strong> Anywhere from three players to thirty — names are
    optional. From five to twelve you start from the game's own deal; change it as much as you
    like. The only thing the tool insists on is one role per player.</li>
    <li><strong>Make the roles yours.</strong> Rename a role, rewrite what its card says, or add
    your own: a Jester, a pair of Lovers, a Serial Killer, a role from your group's house rules.
    Put it on the town, the mafia or an independent side, and choose whether it is shown its
    teammates — the mafia are by default.</li>
    <li><strong>Pass the phone.</strong> The screen shows only whose turn it is. That person taps to
    reveal their card, it hides itself a few seconds later, and they hand it on. Anyone shown their
    team sees exactly who is on it.</li>
    <li><strong>Hand it to your narrator.</strong> Running the game is a person's job. The
    narrator can press and hold to see the whole deal, and nobody else needs to.</li>
  </ol>
</section>
...
<section>
  ...
  the last table you set up, and the roles you made, are remembered on this device and nowhere
  else. It does not run the game, either: it deals the cards and gets out of the way. Turn the
  page into an app from your browser's menu and it keeps working with no signal at all.</p>
</section>
{APP_FUNNEL}
"""
    write("tools/role-generator/index.html", layout(
        ...
        description="Free Mafia role generator for 3 to 30 players. Pass the phone and each player "
                    "sees only their own role; the mafia see their team. Add your own roles in any "
                    "language, including independents. Works offline; nothing leaves your browser.",
        body=body,
    ...
    # The narrator this page used to run was retired: a person runs the game, and the tool only
    # replaces the cards. The address it lived at may be bookmarked or indexed, and GitHub Pages
    # cannot send a real redirect, so it becomes a page that forwards to the role generator and
    # asks not to be indexed. It is written directly, not through layout(), so it never reaches
    # the sitemap.
    target = f"{C.SITE}/tools/role-generator/"
    write("tools/mafia-narrator/index.html", f"""<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Mafia role generator | RPS Mafia</title>
<meta name="robots" content="noindex, follow">
<link rel="canonical" href="{target}">
<meta http-equiv="refresh" content="0; url=/tools/role-generator/">
</head>
<body>
<p>The narrator tool has been retired. Deal roles with the <a href="/tools/role-generator/">Mafia role generator</a>.</p>
</body>
</html>
""")
